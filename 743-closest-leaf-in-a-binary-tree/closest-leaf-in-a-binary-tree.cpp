...
public:
    int findClosestLeaf(TreeNode* root, int k) {
        ...
        // Build graph, and look for the starting node (i.e. root->val == k) at the same time
        ...
        while (!q.empty()) {
            ...
            if (!current->left && !current->right) return current->val; // First leaf node we find, return it
            seen.insert(current);
